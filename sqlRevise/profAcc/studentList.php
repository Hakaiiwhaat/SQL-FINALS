...
<head>
    ...
    <link rel="stylesheet" href="studListStyles.css">
...
<body>
    ...
    <div class="row justify-content-center align-items-center">
        ...
        if (!empty($search_query)) {
            // Concatenate lastName, firstName, and middleName into one column named fullName
            $sql = "SELECT 
                        s.studentID, 
                        ...
                        s.phoneNo, 
                        s.emailAdd,
                        sec.sectionName  -- Fetching the section name from the referenced table
                    FROM 
                        students AS s
                    INNER JOIN 
                        sections AS sec ON s.sectionID = sec.sectionID  -- Joining the students table with the sections table based on the foreign key
                    INNER JOIN
                    ...
                    WHERE 
                        ...
                        (CONCAT(s.lastName, ', ', s.firstName) LIKE '%$search_query%' OR 
                        s.studNo LIKE '%$search_query%')";
        } else {
            // Concatenate lastName, firstName, and middleName into one column named fullName
            $sql = "SELECT 
                        s.studentID, 
                    ...
                    FROM 
                        students AS s
                    INNER JOIN
                        sectionsSubjects AS ss ON s.sectionID = ss.sectionID
                    INNER JOIN
                        subjects AS subj ON ss.subjectID = subj.subjectID
                    INNER JOIN
                        sections AS sec ON s.sectionID = sec.sectionID
                    WHERE
        ...
        echo "<table id='studTable' class = 'table'>";
        echo "<tr>
